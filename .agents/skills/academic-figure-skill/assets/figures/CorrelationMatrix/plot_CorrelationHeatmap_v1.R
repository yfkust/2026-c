# 运行前请先安装依赖：
# install.packages(c("MASS", "GGally", "ggplot2", "dplyr", "patchwork", "ggplotify"))
...
lower_scatter_r2 <- function(data, mapping, ...) {
  x <- GGally::eval_data_col(data, mapping$x)
  y <- GGally::eval_data_col(data, mapping$y)
  r2 <- suppressWarnings(cor(x, y, method = "pearson", use = "complete.obs")^2)

  # 颜色变量稳健选择：优先用最后一列；若失败则退回当前x
  color_var <- NULL
  if (ncol(data) > 0) {
    color_var <- data[[ncol(data)]]
  }
  if (is.null(color_var) || length(color_var) != length(x)) {
    color_var <- x
  }

  ggplot(data = data.frame(x = x, y = y, c = color_var), aes(x, y)) +
    geom_point(aes(color = c), alpha = 0.65, size = 1.1) +
    geom_smooth(method = "lm", se = FALSE, color = "#8a4a2f", linewidth = 0.9) +
    annotate("text", x = min(x, na.rm = TRUE), y = max(y, na.rm = TRUE),
             label = sprintf("R²=%.2f", r2), hjust = 0, vjust = 1, size = 3.3) +
    scale_color_gradientn(colors = c("#08306b", "#2171b5", "#6baed6", "#fdae61", "#d7301f")) +
