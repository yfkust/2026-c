...
for i, method in enumerate(methods):
    # Setting 1的柱子位置
    bar_pos1 = i * width - (len(methods) * width + spacing) / 2
    
    # Setting 2的柱子位置
    bar_pos2 = i * width + (len(methods) * width + spacing) / 2
    
    # 绘制Setting 1的柱子
    bar1 = ax.barh(y_pos[0] + bar_pos1, means_setting1[i], height=width, 
                  xerr=std_setting1[i], capsize=3, alpha=0.8, 
                  color=colors[i], label=method)
    
    # 绘制Setting 2的柱子
    bar2 = ax.barh(y_pos[1] + bar_pos1, means_setting2[i], height=width, 
                  xerr=std_setting2[i], capsize=3, alpha=0.8, 
                  color=colors[i])
    
    # 在柱子右侧显示具体数值
    ax.text(means_setting1[i] - 0.1, y_pos[0] + bar_pos1, f'{means_setting1[i]:.3f}', 
            ha='left', va='center', fontsize=10)
    ax.text(means_setting2[i] - 0.1, y_pos[1] + bar_pos1, f'{means_setting2[i]:.3f}', 
            ha='left', va='center', fontsize=10)
    
    # 添加Setting 1的数据点
    y_points1 = np.random.normal(y_pos[0] + bar_pos1, width/4, size=len(data_points_setting1[i]))
    ax.scatter(data_points_setting1[i], y_points1, color=colors[i], alpha=0.6, s=30, edgecolors='black', linewidth=0.5)
    
    # 添加Setting 2的数据点
    y_points2 = np.random.normal(y_pos[1] + bar_pos1, width/4, size=len(data_points_setting2[i]))
    ...
    if p_values_setting1[i] is not None:
        ax.text(means_setting1[i] + std_setting1[i] + 0.05, y_pos[0] + bar_pos1, 
                f'p={p_values_setting1[i]:.1e}', ha='left', va='center', fontsize=10)
    
    # 添加Setting 2的P值标注
    if p_values_setting2[i] is not None:
        ax.text(means_setting2[i] + std_setting2[i] + 0.05, y_pos[1] + bar_pos1, 
                f'p={p_values_setting2[i]:.1e}', ha='left', va='center', fontsize=10)

# 设置Y轴标签和刻度 - 竖排显示
ax.set_yticks([-0.2, 1.2])  # 两个setting的中心位置
ax.set_yticklabels(settings, rotation=90, va='center')
