...
y_pos = np.array([1, 0])  # Setting 1在位置1，Setting 2在位置0
width = 0.1  # 每个方法在同一个setting中的宽度
spacing = 0.015  # 两个setting之间的间距
...
for i, method in enumerate(methods):
    ...
    bar_offset = (len(methods) - 1 - i) * width - (len(methods) * width) / 2
    
    # 绘制Setting 1的柱子（在上方）
    bar1 = ax.barh(y_pos[0] + bar_offset, means_setting1[i], height=width, 
                  xerr=std_setting1[i], capsize=3, alpha=0.8, 
                  color=colors[i], label=method)
    
    # 绘制Setting 2的柱子（在下方）
    bar2 = ax.barh(y_pos[1] + bar_offset, means_setting2[i], height=width, 
                  xerr=std_setting2[i], capsize=3, alpha=0.8, 
                  color=colors[i])
    
    # 在柱子右侧显示具体数值
    ax.text(means_setting1[i] - 0.1, y_pos[0] + bar_offset, f'{means_setting1[i]:.3f}', 
            ha='left', va='center', fontsize=10)
    ax.text(means_setting2[i] - 0.1, y_pos[1] + bar_offset, f'{means_setting2[i]:.3f}', 
            ha='left', va='center', fontsize=10)
    
    # 添加Setting 1的数据点（上方）
    y_points1 = np.random.normal(y_pos[0] + bar_offset, width/4, size=len(data_points_setting1[i]))
    ax.scatter(data_points_setting1[i], y_points1, color=colors[i], alpha=0.6, s=30, edgecolors='black', linewidth=0.5)
    
    # 添加Setting 2的数据点（下方）
    y_points2 = np.random.normal(y_pos[1] + bar_offset, width/4, size=len(data_points_setting2[i]))
    ...
    if p_values_setting1[i] is not None:
        ax.text(means_setting1[i] + std_setting1[i] + 0.05, y_pos[0] + bar_offset, 
                f'p={p_values_setting1[i]:.1e}', ha='left', va='center', fontsize=7)
    
    # 添加Setting 2的P值标注（下方）
    if p_values_setting2[i] is not None:
        ax.text(means_setting2[i] + std_setting2[i] + 0.05, y_pos[1] + bar_offset, 
                f'p={p_values_setting2[i]:.1e}', ha='left', va='center', fontsize=7)

# 设置Y轴标签和刻度 - 竖排显示
ax.set_yticks([1, 0])  # Setting 1在上方，Setting 2在下方
